...
| **Dev tools** | [mise](https://mise.jdx.dev/) | bat, eza, lazygit, glow, ripgrep, node, bun, gh, glab, direnv, bats, ShellCheck, shfmt |
| **Codex CLI** | Official installer (`run_once_after_14-codex-install.sh.tmpl` on POSIX, `run_once_after_12-codex-windows.ps1.tmpl` on Windows) | [OpenAI Codex CLI](https://github.com/openai/codex) — not mise-managed, see below |
...
On native Windows, winget covers only native prerequisites and helpers such as Git, chezmoi, mise, PowerShell, Git LFS, GnuPG, age, jq, and oh-my-posh. The shared developer CLI set remains mise-managed through `home/dot_config/mise/config.toml`, except for Codex CLI, which is not mise-managed on any platform (mise's bare `codex` shorthand can resolve to either the aqua static-binary backend or the npm backend depending on mise version/platform). `home/run_once_after_12-codex-windows.ps1.tmpl` installs it on native Windows through OpenAI's official one-liner — `powershell -ExecutionPolicy ByPass -Command "irm https://chatgpt.com/codex/install.ps1 | iex"` — and `home/run_once_after_14-codex-install.sh.tmpl` installs it the same way on POSIX/WSL2 — `curl -fsSL https://chatgpt.com/codex/install.sh | sh` (spec 046) — with no npm, no mise, no package manager in between on either platform.
...
└── home/                              # chezmoi source — mirrors $HOME
    ...
    ├── run_once_after_14-codex-install.sh.tmpl     # POSIX Codex CLI via official installer
...
> `codex` is intentionally absent from this manifest — it is not
> mise-managed on any platform (spec 046). `home/run_once_after_14-codex-install.sh.tmpl`
> installs it on POSIX/WSL2 via OpenAI's official one-liner —
> `curl -fsSL https://chatgpt.com/codex/install.sh | sh` — and
> `home/run_once_after_12-codex-windows.ps1.tmpl` installs it the same way
> on native Windows (spec 040). `dotfiles update` / `bin/dotfiles.ps1
> update` re-run the same one-liner on each platform to keep it current,
> the way `mise upgrade` does for every mise-managed tool.
...
- `curl -fsSL https://chatgpt.com/codex/install.sh | sh` — re-runs Codex CLI's official installer (when `codex` is on `PATH` — see spec 046)
